  ...
  const menuItems = [
    {
      ...
      items: [
        ...
        { id: 'pos', label: 'Kasir (Order)', icon: Calculator, badge: 'HOT' },
        { id: 'pesanan', label: 'Pesanan Baru', icon: ShoppingBag, badge: '12' },
    ...
    <>
      ...
      <div className="p-5 flex items-center justify-between border-b border-border-sidebar">
        <a href="#dashboard" className="flex items-center gap-3 no-underline overflow-hidden" onClick={() => setActiveTab('dashboard')}>
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center text-white shrink-0 shadow-lg shadow-blue-600/30">
            <Utensils size={22} />
          </div>
          {!collapsed && (
        ...
        <button
          className="hidden lg:flex w-[30px] h-[30px] rounded-lg items-center justify-center bg-white/5 border border-white/10 text-slate-400 hover:bg-white/15 hover:text-white transition-colors cursor-pointer"
        ...
        {/* Mobile close button */}
        <button
          className="lg:hidden flex w-[30px] h-[30px] rounded-lg items-center justify-center bg-white/5 border border-white/10 text-slate-400 hover:bg-white/15 hover:text-white transition-colors cursor-pointer"
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all cursor-pointer border-none text-left w-full
                    ${isActive
                      ? 'bg-gradient-to-r from-blue-600/20 to-blue-600/5 text-white font-semibold border-l-4 border-blue-500'
